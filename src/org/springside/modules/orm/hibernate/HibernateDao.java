...
public class HibernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK> {
	...
	@SuppressWarnings("unchecked")
	public Page<T> findPage(final Page<T> page,final Map<String, String> alias, final Criterion... criterions) {
		Assert.notNull(page, "page不能为空");

		Criteria c = createCriteria(criterions);
		for(String key:alias.keySet()){
			addAlias((CriteriaImpl)c, alias.get(key), key);
			//c.createAlias(alias.get(key), key);
		}
	...
	public Page<T> findPage(final Page<T> page, final List<PropertyFilter> filters) {
		Criterion[] criterions = buildCriterionByPropertyFilter(filters);
		Map<String,String> alias=buildAliasByPropertyFilter(filters);
		return findPage(page,alias, criterions);
	}
	...
	protected Criterion[] buildCriterionByPropertyFilter(final List<PropertyFilter> filters) {
		...
		for (PropertyFilter filter : filters) {
			if (!filter.hasMultiProperties()) { //只有一个属性需要比较的情况.
				Criterion criterion = buildCriterion(this.getAliasPropertyName(filter.getPropertyName()), filter.getMatchValue(), filter
						.getMatchType());
				criterionList.add(criterion);
			} else {//包含多个属性需要比较的情况,进行or处理.
				Disjunction disjunction = Restrictions.disjunction();
				for (String param : filter.getPropertyNames()) {
					Criterion criterion = buildCriterion(this.getAliasPropertyName(param), filter.getMatchValue(), filter.getMatchType());
	...
	/**
	 * 按属性条件列表创建Alias Map,辅助函数.
	 */
	protected Map<String,String> buildAliasByPropertyFilter(final List<PropertyFilter> filters) {
	...
	protected String getAliasPropertyName(String propertyName){
		if(propertyName.indexOf(".")>0){
			String[] names=StringUtil.split(propertyName, '.');
	 ...
	 * @return
	 */
	public Criteria addAlias(CriteriaImpl criteria, String path, String name){   
		if(path == null) return criteria;   
		...
		return criteria.createAlias(path, name);   
	}
}
