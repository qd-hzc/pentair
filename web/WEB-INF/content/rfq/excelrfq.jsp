<%@ page contentType="application/vnd.ms-excel; charset=UTF-8" %><%@ page import="java.util.*"%><%@ page import="java.util.regex.*"%><%@ page import="java.text.*"%><%@ page import="com.pentair.showcase.rfq.entity.*"%><%@ page import="com.pentair.showcase.common.entity.User"%><%@ page import="jxl.*"%><%@ page import="jxl.write.*"%><%
...
sheet.addCell(new Label(19, 0, "单价<br>（成本）"));
...
for(Rfq rfq : rfqs) {
	...
	if(products.size() > 0) {
		{
			RfqDetail product = products.get(0);
			...
			if(materialCost != null && loh != null) {
				sheet.addCell(new Label(19, row, df.format(materialCost + loh)));
			}
		}
		
		double totalRequire = 0, totalSale = 0;
		for(RfqDetail product : products) {
			...
			if(require != null) {
				...
				if(matcher.find()) {
					totalRequire += Double.parseDouble(matcher.group(0));
				}
			}
			...
			if(sale != null) {
				...
				if(matcher.find()) {
					totalSale += Double.parseDouble(matcher.group(0));
				}
			}
		}
		
		sheet.addCell(new Label(20, row, df.format(totalRequire)));
		sheet.addCell(new Label(21, row, df.format(totalSale)));
	}
